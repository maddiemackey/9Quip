import getRandomInDict from "./getRandomInDict";
...
export default function assignQuips(players = defaultPlayers, prompts = defaultPrompts) {
    let promptsReturned = {};
    let remainder = Object.keys(players).length * 2 % 5;
    let assigningLimit = remainder > 0 ? Math.floor(Object.keys(players).length * 2 / 5) + 1 : Math.floor(Object.keys(players).length * 2 / 5);
    let randPrompts = getRandomInDict(prompts, assigningLimit);
